...
class Extractor:
    ...
    def get_text(r):
        ...
        if text_element is not None:
            ...
            if text_element is not None:
                ...
                text = (
                    text_element.get_text().strip() if text_element is not None else ""
                )
    ...
    def get_date_string(r, page):
        ...
        if (date_string == "" or date_string is None) and date_element is not None:
            ...
            if span_element is not None:
                ...
                date_index = (
                    use_element["xlink:href"] if use_element is not None else None
                )
                date_element = (
                    page.find("text", {"id": str(date_index)})
                ...
                if date_element is None:
                    ...
                    if date_element is None:
                        ...
                        date_index = (
                            svg_element["xlink:href"]
                            if svg_element is not None
                            else None
                        )
                        svg_date_element = (
                            page.find("svg", {"id": str(date_index[1:])}).find("use")
                            if date_index is not None
                            else None
                        )
                        svg_date_index = (
                            svg_date_element["xlink:href"]
                            if svg_date_element is not None
                            else None
                        )
                        # print(svg_date_index)
                        date_element = (
                            page.find("text", {"id": str(svg_date_index[1:])})
                            if svg_date_index is not None
                            else None
                        )
    ...
    def extract_data(self, page_source=None, source_file=None):
        ...
        group_posts = page.find_all("div", {"class": tag.post_list_tag})
