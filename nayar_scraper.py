...
def run(
    ...
    fb_scraper = FacebookScraper(
        credentials, driver_location, use_cookies=use_cookie, raw_data_dir=raw_data_dir
    )
    extractor = Extractor()
    # Main loop
        ...
        try:
            ...
            try:
                ...
                if len(df) == 0:
                    ...
                    # Write data to csv
                    extractor.write_csv(
                        df, f"{folder_path}/group_{group_id}_{len(df)}.csv"
                    )
            except:
                pass
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--group_file_path",
        type=str,
        default="/data/nayar_public_active_groups.csv",
        help="Path of group csv file.",
    )
    parser.add_argument(
        "--output_path",
        type=str,
    ...
    parser.add_argument(
        "--resume", action="store_true", default=False, help="Enable resume mode."
    )
    parser.add_argument(
        "--use_cookies",
        action="store_true",
        default=False,
        help="Enable cookies usage.",
    )
    parser.add_argument(
        "--date", type=str, default=None, help="Date in format YYYY-MM-DD."
    )
    parser.add_argument(
        "--extra_date",
        type=str,
        default=None,
        help="Extra date in format YYYY-MM-DD if needed. Use comma if multiple date.",
    )
    parser.add_argument(
        "--posts_lookup", type=int, default=200, help="Number of posts to check."
    )
    parser.add_argument(
        "--credentials",
        type=str,
        default="credentials.txt",
        help="Path of credentials file in .txt format.",
    )
    parser.add_argument(
        "--driver_location",
        type=str,
        default="local",
        help="Path of chrome driver location egs. docker, local.",
    )
